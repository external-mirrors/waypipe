...
/* Like `format!`, but prepends file and line number. tag!("Failed to X: {} {}", arg1, arg2) */
#[macro_export]
macro_rules! tag {
    ($x:tt) => {
...
/* Invoke self via new process. (Instead of by forking. Spawning ensures
 * ASLR is reseeded, and provides a nicer command line string, with the
 * mixed-cost-benefit of using the latest dynamic library versions, and
 * repeating process setup costs.) */
...
fn launch_oneshot_client_inner(
    channel_socket: &OwnedFd,
    command: Option<&[&OsStr]>,
) -> Result<(Option<std::process::Child>, OwnedFd), String> {
...
fn run_client_oneshot(
    ...
    let (cmd_child, link_fd) = launch_oneshot_client_inner(&channel_socket, command)?;
...
/* Connect to upstream, create a security context, and change the target of our upstream connections */
fn setup_secctx(
    cwd: &OwnedFd,
    app_id: &str,
    wayland_socket: Option<OwnedFd>,
...
/* Returns index of hostname in ssh_args, and whether ssh will force pseudo-terminal allocation */
